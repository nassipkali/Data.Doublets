namespace Platform::Data::Doublets {
  ...
  template <typename TStorage>
  static typename TStorage::LinkAddressType GetSource(const TStorage& storage, typename TStorage::LinkAddressType linkAddress) {
  ...
  template <typename TStorage>
  static typename TStorage::LinkAddressType GetTarget(const TStorage& storage, typename TStorage::LinkAddressType linkAddress) {
  ...
  static std::string Format(TStorage& storage, const typename TStorage::LinkType& link) {
    constexpr auto constants = storage.Constants;
    return std::string{}.append("(").append(std::to_string(GetIndex(storage, link))).append("): ").append("(").append(std::to_string(GetSource(storage, link))).append(")").append(" -> ").append("(").append(std::to_string(GetTarget(storage, link))).append(")");
  }
  ...
  static void DeleteByQuery(TStorage& storage, typename TStorage::LinkType query) {
    ...
    DIRECT_METHOD_CALL(TStorage, storage, Each, query, [&](typename TStorage::LinkType vec) -> typename TStorage::LinkAddressType { return storage.Constants.Continue; });
