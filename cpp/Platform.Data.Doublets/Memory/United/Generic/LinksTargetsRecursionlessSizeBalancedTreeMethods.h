  ...
  class LinksTargetsRecursionlessSizeBalancedTreeMethods : public LinksRecursionlessSizeBalancedTreeMethodsBase<LinksTargetsRecursionlessSizeBalancedTreeMethods<TLinksOptions>, TLinksOptions> {
   ...
   public:
    LinkAddressType* GetLeftReference(LinkAddressType node) {
      return &this->GetLinkReference(node).LeftAsTarget;
    }

   public:
    LinkAddressType* GetRightReference(LinkAddressType node) {
      return &this->GetLinkReference(node).RightAsTarget;
    }
   ...
   public:
    void SetLeft(LinkAddressType node, LinkAddressType left) {
      this->GetLinkReference(node).LeftAsTarget = left;
    }

   public:
    void SetRight(LinkAddressType node, LinkAddressType right) {
      this->GetLinkReference(node).RightAsTarget = right;
    }

   public:
    LinkAddressType GetSize(LinkAddressType node) {
      return this->GetLinkReference(node).SizeAsTarget;
    }

   public:
    void SetSize(LinkAddressType node, LinkAddressType size) {
      this->GetLinkReference(node).SizeAsTarget = size;
    }

   public:
    LinkAddressType GetTreeRoot() {
      return this->GetHeaderReference().RootAsTarget;
    }

   public:
    LinkAddressType GetBasePartValue(LinkAddressType linkAddress) {
      return this->GetLinkReference(linkAddress).Target;
    }
